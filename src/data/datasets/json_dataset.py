...
class JSONDataset(torch.utils.data.Dataset):
    ...
    def _construct_imdb(self, cfg):
        """Constructs the imdb."""
        if cfg.DATA.NAME == "multi":
            class_nbr = {"FGVC-cars": 196, "FGVC-cub": 200, "FGVC-dogs": 120, "FGVC-flowers": 102, "FGVC-nabirds": 555}
            label_counter = 0
            self._imdb = []
            self._class_ids = []
            for name in class_nbr.keys():
                class_dir = self.get_imagedir() + name
                if add_dict[name] != "":
                    img_dir = os.path.join(class_dir, add_dict[name])
                else:
                    img_dir = os.path.join(class_dir)
                anno_path = os.path.join(class_dir, "{}.json".format(self._split))
                anno = read_json(anno_path)
                class_ids_list = sorted(list(set(anno.values())))
                class_id_cont_id_dict = {v: i for i, v in enumerate(class_ids_list)}
                # label_count = len(list(set(anno.values())))
                label_count = class_nbr[name]
                for img_name, cls_id in anno.items():
                    cont_id = class_id_cont_id_dict[cls_id]
                    im_path = os.path.join(img_dir, img_name)
                    self._imdb.append({"im_path": im_path, "class": cont_id+label_counter})
                label_counter += label_count
                self._class_ids += class_ids_list
                #print(name, label_count)
            #print(cfg.DATA.NUMBER_CLASSES, label_counter)
            assert(cfg.DATA.NUMBER_CLASSES == label_counter)
...
class MultiDataset(JSONDataset):
    """Multitask dataset."""
    def __init__(self, cfg, split):
        super(MultiDataset, self).__init__(cfg, split)
